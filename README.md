...
#### Класс MainPage
Отвечает за главную страницу, содержит кнопку корзины. Принимает в конструктор  - элемент разметки  корзины и экземпляр `EventEmitter` для инициации событий нажатия пользователем на корзину. Так же предоставляет сеттер `container` для полного обновления содержимого контейнера карточек товара.
...
- `cartItemCounter: canged` - изменение данных счетчика корзины
